...
Self-hosting files added to this repo:

| File | Purpose |
| --- | --- |
| `docker-compose.yml` | Runs self-hostable services: builds/runs the Next.js web server, Postgres, cron caller, Postgres backups, and Watchtower. |
| `infra/self-host/.env.example` | Copyable environment template for the self-hosted stack, defaulting to the locally built `fx-trader-profiles-web:local` image. |
| `scripts/self-host-setup.sh` | Bootstraps `.env`, generates `CRON_SECRET` if missing, builds the web image, pulls support images, and starts the compose stack. |
| `scripts/self-host-monitor.sh` | Checks compose service state, container health, Postgres readiness, app `/api/health`, and resource usage. |

Watchtower is configured with `--label-enable`, so only containers labeled with `com.centurylinklabs.watchtower.enable=true` are auto-updated. This keeps Postgres from being updated unexpectedly while still allowing the app image to roll forward.

The web server is now scaffolded under `apps/web` and has its own Dockerfile at `apps/web/Dockerfile`. The default compose path builds `fx-trader-profiles-web:local` directly from the repo, so a VPS can run the full self-hostable stack before a registry-published image exists.
