...
| File | Purpose |
| --- | --- |
| `docker-compose.yml` | Runs self-hostable services: app container, Postgres, cron caller, Postgres backups, and Watchtower. |
| `infra/self-host/.env.example` | Copyable environment template for the self-hosted stack. |
| `scripts/self-host-setup.sh` | Bootstraps `.env`, generates `CRON_SECRET` if missing, pulls images, and starts the compose stack. |
