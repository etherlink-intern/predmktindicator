      ...
      const [generatedAt, walletMaintenance, totalsResult, poolsResult, averageEntryBookResult, tradersResult] = await Promise.all([
        ...
        client.query<Record<string, unknown>>(`
          with base_positions as (
            ...
            select
              p.instrument,
              case when p.instrument = 'BTC' then 5000::numeric else 200::numeric end as bucket_size_usd,
              floor(p.avg_entry_price_usd / (case when p.instrument = 'BTC' then 5000::numeric else 200::numeric end)) * (case when p.instrument = 'BTC' then 5000::numeric else 200::numeric end) as bucket_low_usd,
              p.side,
              p.owner,
              p.notional_usd
            from positioned_entries p
            where p.avg_entry_price_usd is not null
